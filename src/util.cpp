...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NiCKEL";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "NiCKEL.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "NiCKELd.pid"));
...
void AddTimeData(const CNetAddr& ip, int64 nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (abs64(nMedian) < 15 * 60) // NiCKEL: changed maximum adjust to 15 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        }
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong NiCKEL will not work properly.");
